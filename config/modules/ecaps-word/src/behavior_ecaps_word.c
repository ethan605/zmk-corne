...
static int on_ecaps_word_binding_pressed(struct zmk_behavior_binding *binding,
...
static int on_ecaps_word_binding_released(struct zmk_behavior_binding *binding,
                                          struct zmk_behavior_binding_event event) {
    return ZMK_BEHAVIOR_OPAQUE;
}
...
static bool ecaps_word_list_contains(const struct ecaps_word_key_list *list, uint16_t usage_page,
    ...
    for (int i = 0; i < list->size; i++) {
        ...
        if (key->page == usage_page && key->id == usage_id &&
            (key->implicit_modifiers & (implicit_modifiers | zmk_hid_get_explicit_mods())) ==
...
static bool ecaps_word_should_shift(const struct behavior_ecaps_word_config *config,
                                    struct zmk_keycode_state_changed *ev) {
    if (ev->usage_page == HID_USAGE_KEY && ecaps_word_is_alpha(ev->keycode)) {
        return true;
    }
    return ecaps_word_list_contains(config->shift_list, ev->usage_page, ev->keycode,
                                    ev->implicit_modifiers);
}

// Continue the word for alpha, numeric, modifiers, shift-list (implies continue), and continue-list.
static bool ecaps_word_should_continue(const struct behavior_ecaps_word_config *config,
                                       struct zmk_keycode_state_changed *ev) {
    if (is_mod(ev->usage_page, ev->keycode) || ecaps_word_should_shift(config, ev)) {
        return true;
    }
    if (ev->usage_page == HID_USAGE_KEY && ecaps_word_is_numeric(ev->keycode)) {
        return true;
    }
    return ecaps_word_list_contains(config->continue_list, ev->usage_page, ev->keycode,
                                    ev->implicit_modifiers);
}
...
static int ecaps_word_keycode_state_changed_listener(const zmk_event_t *eh) {
    ...
    for (int i = 0; i < ARRAY_SIZE(devs); i++) {
        ...
        if (!ecaps_word_should_continue(config, ev)) {
            LOG_DBG("Deactivating ecaps_word for 0x%02X - 0x%02X", ev->usage_page, ev->keycode);
            deactivate_ecaps_word(dev);
        }
    }
...
#define KP_INST(n)                                                                                 \
    ...
    static const struct ecaps_word_key_list ecaps_word_shift_##n = PROP_KEY_LIST(n, shift_list);   \
    static struct behavior_ecaps_word_data behavior_ecaps_word_data_##n = {.active = false};       \
    static const struct behavior_ecaps_word_config behavior_ecaps_word_config_##n = {              \
    ...
    BEHAVIOR_DT_INST_DEFINE(n, NULL, NULL, &behavior_ecaps_word_data_##n,                          \
                            ...
                            CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &behavior_ecaps_word_driver_api);
